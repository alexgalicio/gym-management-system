    ...
    <form role="form" action="index.php" method="POST">
      ...
      if (isset($_POST['fullname'])) {
        ...
        if ($type == 'Student' || $type == 'Walk-in') {
          ...
          $amount = 300;
          $status = 'Active';
          $membership_end = date("Y-m-d", strtotime("+12 months", strtotime($membership_start)));
        }

        include 'dbcon.php';

        // get user current password
        $qry1 = "SELECT password FROM members WHERE user_id='$id'";
        ...
        $qry = "update members set fullname='$fullname', username='$username', password='$password', amount='$amount', gender='$gender', type='$type', address='$address', contact='$contact', email='$email', status='$status', membership_start='$membership_start', membership_end='$membership_end'  where user_id='$id'";
        ...
        if (!$result) {
          ...
          echo "<a class='btn btn-warning btn-big'  href='members.php'>Go Back</a> </div>";
          echo "</div>";
          echo "</div>";
          echo "</div>";
          echo "</div>";
